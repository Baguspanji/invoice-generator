...
@section('title', 'Ringkasan Keuangan — IceSum')

@section('content')
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h1 class="text-2xl font-bold text-dark dark:text-white">Ringkasan Keuangan</h1>
        <form method="GET" action="{{ route('dashboard') }}" class="flex items-center gap-2">
            <select name="year" onchange="this.form.submit()"
                class="rounded-lg border border-slate-200 bg-white py-2 pl-3 pr-8 text-sm focus:border-primary focus:ring-primary dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div
            ...
            <h3 class="mb-4 text-lg font-bold text-dark dark:text-white">Grafik Pendapatan Bulanan</h3>
        ...
        <div
            class="flex flex-col rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900 lg:col-span-1">
            <h3 class="mb-4 text-lg font-bold text-dark dark:text-white">Invoice Terbaru</h3>
            <div class="flex-1 space-y-4">
                @forelse ($recentInvoices as $invoice)
                    <div
                        class="flex items-center justify-between border-b border-slate-100 pb-3 last:border-0 last:pb-0 dark:border-slate-700">
                        <div>
                            <p class="text-sm font-semibold text-dark dark:text-white">{{ $invoice->invoice_number }}</p>
                            <p class="text-xs text-slate-500 dark:text-slate-400">{{ $invoice->customer->name ?? '-' }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-sm font-semibold text-dark dark:text-white">Rp
